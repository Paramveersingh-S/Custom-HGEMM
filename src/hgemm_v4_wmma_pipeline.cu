...
__global__ void hgemm_v4_wmma_pipeline_kernel(const half* A, const half* B, half* C, int M, int N, int K) {
    int bx = blockIdx.x;
    int by = blockIdx.y;
    int tid = threadIdx.x;
    int warpId = tid / 32;
    ...
    // To use cp.async, we copy 16 bytes (8 halfs) at a time
    // For A: 128x32 = 4096 elements. Each thread copies 16 elements (2 * 16 bytes)
    for (int i = 0; i < 2; i++) {
        int linear_idx = (tid + i * 256) * 8; // Start index for this 8-element vector
    ...
    for (int k = 0; k < num_k_tiles; k++) {
        ...
        cp_async_wait_group<1>(); // Wait until at most 1 group is in flight (i.e. the one we just prefetched)
        __syncthreads();
        ...
        for (int step = 0; step < BK; step += 16) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                wmma::load_matrix_sync(b_frag[j], &s_B[cur][step][smem_n], BN);
            }
    ...
    // Wait for the last prefetched tile to finish before we potentially overwrite shared memory with C tiles
...
void launch_hgemm_v4_wmma_pipeline(const half* A, const half* B, half* C, int M, int N, int K) {
    ...
    hgemm_v4_wmma_pipeline_kernel<<<grid, block>>>(A, B, C, M, N, K);
